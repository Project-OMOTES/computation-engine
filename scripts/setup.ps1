...
docker compose --profile=manual_dev down omotes_postgres_db_dev
docker compose up -d --wait postgres_db
docker compose exec omotes_postgres_db psql -d postgres -c 'CREATE DATABASE omotes_jobs;'
docker compose exec omotes_postgres_db psql -d omotes_jobs -c "CREATE USER $env:POSTGRES_ORCHESTRATOR_USER_NAME WITH PASSWORD '$env:POSTGRES_ORCHESTRATOR_USER_PASSWORD';"
docker compose exec omotes_postgres_db psql -d omotes_jobs -c "ALTER USER $env:POSTGRES_ORCHESTRATOR_USER_NAME WITH PASSWORD '$env:POSTGRES_ORCHESTRATOR_USER_PASSWORD';"
docker compose exec omotes_postgres_db psql -d omotes_jobs -c "GRANT ALL PRIVILEGES ON DATABASE omotes_jobs TO $env:POSTGRES_ORCHESTRATOR_USER_NAME;"
...
docker compose build omotes_postgres_db_upgrade
docker compose run --rm omotes_postgres_db_upgrade
